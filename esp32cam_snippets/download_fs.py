...
class LittleFSInfo(FSInfo):
    def __init__(self, start, length, page_size, block_size):
        ...
        self.tool = join(platform.get_package_dir(
            "tool-mklittlefs"), self.tool)
...
def get_fs_type_start_and_length():
    ...
    if platform == "espressif32":
        ...
        if filesystem not in ("spiffs", "littlefs"):
            print("Unrecognized board_build.filesystem option '" +
                  str(filesystem) + "'.")
            env.Exit(1)
...
def download_fs(fs_info: FSInfo):
    esptoolpy = join(platform.get_package_dir(
        "tool-esptoolpy") or "", "esptool.py")
    fs_file = join(
        env["PROJECT_DIR"], f"downloaded_fs_{hex(fs_info.start)}_{hex(fs_info.length)}.bin")
    esptoolpy_flags = [
        "--chip", mcu,
    ...
    esptoolpy_cmd = '"' + env["PYTHONEXE"] + '"' + \
        ' "' + esptoolpy + '" ' + " ".join(esptoolpy_flags)
...
def unpack_fs(fs_info: FSInfo, downloaded_file: str):
    ...
    try:
        ...
        print("Exception while attempting to remove the folder '" +
              str(unpack_dir) + "': " + str(exc))
...
def command_download_fs(*args, **kwargs):
    ...
    print("Download was okay: " + str(download_ok) +
          ". File at: " + str(downloaded_file))
