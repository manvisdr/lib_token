...
boolean detectedShort = false;
boolean detectedLong = false;
int detectPrint = 0;
int detectLongCnt = 0;
int detectShortCnt = 0;
int detectCnt = 0;
...
void setup(void)
{
  ...
  //     .data_in_num = 15   // DOUTESP32-INMP441 wiring
  const i2s_pin_config_t pin_config = {
...
void loop(void)
{
  ...
  else if (detectShort) // and !detectLong)
  {
    detectPrint = 1;
    detectCnt++;
    detectShortCnt++;
  ...
  if (detectLongCnt > 35)
    detectedLong = true;
  else if (detectShortCnt > 1 and detectShortCnt < 35)
    detectedShort = true;
  else
  {
    detectedLong = false;
  ...
  // if (detectFrequency(&bell, 22, peak, 117, 243, true))
  // {

  //   Serial.printf("counter - %d | peak - %d\n", cnts++, peak);
  //   Serial.println("DETEK LONG BEEP");

  //   // sendAlarm(0, "home/alarm/doorbell", 2000);
  // }
  // else if (detectFrequency(&bell, 2, peak, 117, 243, true))
  // {

  //   Serial.printf("counter - %d | peak - %d\n", cnts++, peak);
  //   Serial.println("DETEK SHORT BEEP");
