...
void MqttTopicInit()
{
  ...
  sprintf(topicREQNotif, "%s/%s/%s/%s", mqttChannel, "request", idDevice, "notif");
  ...
  // sprintf(topicRSPSoundVal, "%s/%s/%s/%s", mqttChannel, "respond", idDevice, "sound/value");
  // Serial.printf("%s\n", topicREQToken);
  // Serial.printf("%s\n", topicREQView);
  // Serial.printf("%s\n", topicRSPSignal);
  // Serial.printf("%s\n", topicRSPStatus);
...
bool SoundDetectShortItron()
{
  ...
  Serial.printf("%d %d %d %d %d\n", SoundPeak, detectedShortItronFast, detectShortCnt);
  ...
  else
  {
    ...
    if (detectedShortItronFast and loudness > 30) // and !detectLong)
    {
      cntDetected++;
      detectShortCnt++;
    }
...
void TokenProcess(/*char *token*/)
{
  ...
  if (status == Status::PROCCESS_TOKEN and millis() < afterSolenoid + kwhtimestatbuff)
  {
        ...
        millis() < afterSolenoid + kwhtimestatbuff and TokenStateStatus)
    {
      ...
      String outputvalid = OutputSoundValidasi(soundValidasiCnts);
